...
class DnCNN(NNRegressor):
    ...
    def forward(self, x):
        ...
        self.n.append(h.shape[-1])
        for i in range(D):
            h = F.relu(self.bn[i](self.conv[i+1](h)))
            self.n.append(h.shape[-1])
        y = self.conv[D+1](h) + x
        self.n.append(y.shape[-1])
        return y
...
class DnCNNNoBN(NNRegressor):

    def __init__(self, D, C=64):
        super(DnCNNNoBN, self).__init__()
        self.D = D
        self.n = []
        # convolution layers
        self.conv = nn.ModuleList()
        self.conv.append(nn.Conv2d(3, C, 3, padding=1,padding_mode='circular'))
        self.conv.extend([nn.Conv2d(C, C, 3, padding=1,padding_mode='circular') for _ in range(D)])
        self.conv.append(nn.Conv2d(C, 3, 3, padding=1,padding_mode='circular'))
        # apply He's initialization
        for i in range(len(self.conv[:-1])):
            nn.init.kaiming_normal_(
                self.conv[i].weight.data, nonlinearity='relu')
