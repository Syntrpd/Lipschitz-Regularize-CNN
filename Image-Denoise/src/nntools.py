...
from Utilities import computeLayerLipschitzFourier
...
class Experiment(object):
    ...
    def run(self, num_epochs, plot=None):
        ...
        x_test,d_test = next(iter(self.train_loader))
        for i in range(x_test.shape[0]):
            im_x = torch.clamp(torch.permute(untransform(x_test[i,:,:,:]),(1,2,0)),min=0,max=1)
            ...
            psnr = 10*np.log10(im_x.shape[0]*im_x.shape[1]*im_x.shape[2]*255*255/np.sum(np.square(img-dst)))
            print(f"PSNR NLM Img{i}: {psnr}")
            #print(dst)
        ...
        for epoch in range(start_epoch, num_epochs):
            ...
            for x, d in self.train_loader:
                ...
                #if(self.epoch<=100):
                #loss = self.net.alt_criterion(y, d)
            ...
            if((self.epoch%20)==0):
                y_test = self.net.forward(x_test.to(self.net.device)).cpu().detach()
                
                
                for i in range(y_test.shape[0]):
                    im_y = torch.clamp(torch.permute(untransform(y_test[i,:,:,:]),(1,2,0)),min=0,max=1)
                    im_d = torch.clamp(torch.permute(untransform(d_test[i,:,:,:]),(1,2,0)),min=0,max=1)
                    psnr = 10*torch.log10(im_y.shape[0]*im_y.shape[1]*im_y.shape[2]/torch.sum(torch.square(im_y-im_d)))
